...
class SummarizeWorkerTest {
    ...
    fun `worker returns retry when no model is present`() = runBlocking {
        ...
        val worker = TestListenableWorkerBuilder<SummarizeWorker>(ctx)
            ...
            .setWorkerFactory(
                TestSummarizeWorkerFactory(repo, client, modelManager, ContentHintDetector(), NotificationHelper(ctx), FakeOcrEngine(), store, com.example.aiinbox.sync.SyncCoordinator(ctx))
            )
            .build()
