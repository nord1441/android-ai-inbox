...
class ModelManagerTest {
    ...
    fun `model not present initially`() {
        assertThat(manager.isModelPresent(ModelVariant.GEMMA_3_1B)).isFalse()
    }

    @Test
    fun `model file path is under no-backup files dir`() {
        val path = manager.modelFilePath(ModelVariant.GEMMA_3_1B)
    ...
    fun `delete removes the file`() {
        ...
        assertThat(manager.isModelPresent(ModelVariant.GEMMA_3_1B)).isTrue()
        manager.deleteModel(ModelVariant.GEMMA_3_1B)
        assertThat(manager.isModelPresent(ModelVariant.GEMMA_3_1B)).isFalse()
    }
    ...
    fun `currentVariant returns variant whose file exists`() {
        ...
        path.parentFile?.mkdirs()
        path.writeBytes(ByteArray(16))
        assertThat(manager.currentVariant()).isEqualTo(ModelVariant.GEMMA_3_1B)
    }
}
