...
1. **画像共有受信**
   ...
   - テキストと画像の同時共有にも対応
2. **スクリーンショット取り込み**
   - 端末の「アクションキー割当」「Quick Settings タイル」「アプリドロワー上の専用アイコン (📸 スクショ to AI Inbox)」のいずれからでも1タップで撮影 → Inbox 化
   - `MediaProjection` API による撮影（毎回 OS の同意ダイアログを伴う）
...
| スクショ撮影方式 | MediaProjection のみ (A) | OS のセキュリティモデルに従順、Play 審査がスムーズ、毎回同意は受容 |
...
├── screenshot/                           ← 新規パッケージ
│   ├── ScreenshotCaptureActivity.kt      ← Theme.NoDisplay。MediaProjection 同意取得→Service起動
│   ├── ScreenshotCaptureService.kt       ← Foreground Service。VirtualDisplay で1フレ撮影
│   └── ScreenshotTileService.kt          ← Quick Settings タイル
├── ocr/                                  ← 新規パッケージ
...
#### `ScreenshotCaptureActivity`（`Theme.NoDisplay`、Launcher 別エントリ「📸 スクショ to AI Inbox」）

- `MediaProjectionManager.createScreenCaptureIntent()` を `registerForActivityResult` で投げる
- 同意取れたら `ScreenshotCaptureService` を `startForegroundService()` で起動、結果トークンを Intent extra で渡す
- 直後 `finish()`（Toast はサービス側で表示）

#### `ScreenshotCaptureService`（`foregroundServiceType="mediaProjection"`）

- `MediaProjection` を構築 → `ImageReader` + `VirtualDisplay` で1フレ取得
- Bitmap → JPEG q85 / 長辺2048pxに正規化 → `EncryptedImageStore.save()`
- `InboxRepository.createPendingItemWithAttachments(text=null, attachments=[...], sourceApp="screenshot:capture")`
- `WorkScheduler.enqueueSummarize(itemId)`
- Toast「保存しました」→ `stopSelf()`
- `MediaProjection.Callback.onStop()` で `VirtualDisplay` リーク防止
- `ImageReader` は1フレ取れたら即 `close()`（ストリーミングしない）
- 取得タイミングが遅いと「同意ダイアログを閉じた直後の黒画面」を撮ってしまう可能性がある → 200ms ディレイ + リトライ1回

#### `ScreenshotTileService`（`TileService` API）

- タイルタップ → `startActivityAndCollapse(Intent(ScreenshotCaptureActivity))`
- ロック画面からの起動許可は端末設定依存
...
### 6.1 アクションキー / Quick Settings → スクショ撮影 → Inbox

```
[アクションキー押下 / Quick Settings タップ / アプリドロワー「📸」タップ]
        ↓
ScreenshotCaptureActivity.onCreate()
  ├─ MediaProjectionManager.createScreenCaptureIntent()
  ├─ registerForActivityResult(StartActivityForResult) で投げる
  │     ↓
  │  [システム同意ダイアログ「録画を許可しますか？」]
  │     ↓ ユーザー承諾
  ├─ result data + resultCode を Intent extra に詰めて
  ├─ startForegroundService(ScreenshotCaptureService)
  └─ finish()                                    ← Activity は即終了

ScreenshotCaptureService.onStartCommand()
  ├─ startForeground(notif, mediaProjection)    ← Android 14+ 必須
  ├─ MediaProjection.createVirtualDisplay(ImageReader.surface)
  ├─ ImageReader.OnImageAvailableListener:
  │     ├─ acquireLatestImage() → Bitmap
  │     ├─ VirtualDisplay/MediaProjection を即解放（次フレ撮らない）
  │     ├─ 黒画面検出（平均輝度 < 5）→ 200ms 後に再試行 1 回
  │     ├─ Bitmap → JPEG q85, 長辺2048px に正規化
  │     ├─ EncryptedImageStore.save(bytes) → "<uuid>.jpg.enc"
  │     ├─ Repository.createPendingItemWithAttachments(
  │     │       text=null, subject=null, sourceApp="screenshot:capture",
  │     │       attachments=[Attachment(kind=SCREENSHOT, ...)])
  │     ├─ WorkScheduler.enqueueSummarize(itemId)
  │     ├─ Toast「保存しました」
  │     └─ stopSelf()
  └─ （タイムアウト 10s 経過しても画像取得できなければ Toast「失敗」+ stopSelf）
```
...
既存の `FOREGROUND_SERVICE` は `ScreenshotCaptureService` でも引き続き使用。
...
<!-- ScreenshotCaptureActivity（Launcher 別エントリ） -->
<activity
    ...
    android:label="@string/screenshot_launcher_label"   <!-- 「📸 スクショ to AI Inbox」 -->
...
| ケース | 挙動 |
|---|---|
| MediaProjection 同意拒否 | Activity result が `RESULT_CANCELED` → Toast「キャンセルしました」、DB書き込みなし |
| MediaProjection 取得後にユーザーが「停止」通知をタップ | `Callback.onStop()` で `stopSelf()`、撮影未完了なら DB 書き込みなし |
| ImageReader が一定時間（10秒）画像を返さない | Toast「スクリーンショットに失敗しました」、`stopSelf()` |
| MediaProjection 取得直後の黒画面検出 | 画像の平均輝度 < 5 なら 200ms 後にリトライ1回。それでも黒なら通常保存（誤判定回避） |
...
| Quick Settings タイルからロック画面起動時 | OS 仕様により先にデバイスアンロックが要求される（Android 12+） — 仕様として受け入れる |
| アクションキーが無い端末 | アプリドロワー / Quick Settings タイル経由で同じ Activity が起動可能 |
...
- `MediaProjection`、`VirtualDisplay`、`ImageReader`、`Bitmap` は必ず `try { ... } finally { close()/release() }` で解放
- ScreenshotCaptureService は撮影完了 / タイムアウトで必ず `stopSelf()` するよう finally で保証
...
- アクセシビリティサービスによるサイレントスクショ撮影 — ヘビーユーザー向けオプトイン
- 動画 / GIF / PDF の取り込み
- スクショの編集（クロップ・注釈）
